<template>
	<div :class="$style.signPop" v-if="pop">
		<div v-if="type=='up'" :class="$style.signup">
			<div :class="$style.mobileTab" v-if="MobilePop">
				<h2 @click.prevent="MobilTypeChange('in')">Sign in</h2>
				<h2 @click.prevent="MobilTypeChange('up')">Sign up</h2>
			</div>
			<h2 v-if="!MobilePop">Sign Up</h2>
		...
		<div v-if="type=='in'" :class="$style.signin">
			<div :class="$style.mobileTab" v-if="MobilePop">
				<h2 @click.prevent="MobilTypeChange('in')">Sign in</h2>
				<h2 @click.prevent="MobilTypeChange('up')">Sign up</h2>
			</div>
			<h2 v-if="!MobilePop">Sign In</h2>
...
<script>
	...
	export default {
		...
		methods : {
			...
			MobilTypeChange : function(MobileType) {
				if(MobileType == 'in') {
...
<style module>
	...
	.signPop>div form>input.submit {width:100%; height:40px; border:0; cursor:pointer; background-color:#005944; border-radius:5px; margin-bottom:10px; color:#fff;}
	...
	@media screen and (max-width:800px) {
		.signPop {z-index:50;}
		.signPop>div {width:90vw; padding:2.5vw; overflow:hidden;}
		.signPop>div .mobileTab {padding:2.5vw 0; margin-bottom:1.5vw; padding-bottom:1.5vw; overflow:hidden; text-align:center;}
		.signPop>div .mobileTab>h2 {width:49%; line-height:8vw; border-radius:1vw; box-sizing:border-box; color:#005944; border:1px solid #005944; font-size:4.5vw;}
		.signPop>div .mobileTab>h2:first-child {float:left;}
		.signPop>div .mobileTab>h2:last-child {float:right;}
		.signPop>div.signup .mobileTab>h2:last-child {background-color:#005944; color:#fff;}
		.signPop>div.signin  .mobileTab>h2:first-child {background-color:#005944; color:#fff;}
		...
		.signPop>div form>input.cancel {line-height:5vw; font-size:4vw;}
		.signPop>div form>input.submit {margin-bottom:1.5vw; font-size:4vw;}
		.signPop>div ul {margin-top:2.5vw; padding-top:1.5vw; margin-bottom:2vw;}
		.signPop>div ul li {font-size:3vw;}
		.signPop>div>p {float: right;; margin:3vw 3vw 0;}
	}
</style>
